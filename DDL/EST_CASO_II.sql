DROP SCHEMA if exists EST_CASO_II;

create schema EST_CASO_II;

use EST_CASO_II;

create table if not exists PASSAGEIROS (

  PASS_CPF INT PRIMARY KEY AUTO_INCREMENT,
  PASS_NOME VARCHAR(200) NOT NULL,
  PASS_TELEFONE INT,
  PASS_RUA VARCHAR(45) NOT NULL,
  PASS_NUMRUA INT,
  PASS_BAIRRO VARCHAR(45) NOT NULL,
  PASS_CIDADE VARCHAR(45) NOT NULL,
  PASS_ESTADO VARCHAR(45) NOT NULL,
  PASS_PAIS VARCHAR(30) NOT NULL,
  PASS_CODPOSTAL VARCHAR(10) NOT NULL, 
  PASS_EMAIL varchar(200)
);

create table if not exists AEROPORTOS(

  AEROPORTO_COD int primary key AUTO_INCREMENT,
  AEROPORTO_NOME varchar(100) not null,
  AEROPORTO_LOCAL varchar(100) not null,
  AEROPORTO_PAIS varchar(50) not null,
  AEROPORTO_LONGITUDE float,
  AEROPORTO_LATITUDE float
);

create table if not exists AERONAVES(

  AERONAVE_PREFIXO int primary key AUTO_INCREMENT,
  AERONAVE_MODELO varchar(100) not null,
  AERONAVE_ANOFABR date not null,
  AERONAVE_CAPACIDADE int not null,
  AERONAVE_FABRICANTE varchar (50)
);

create table if not exists FUNCIONARIOS(

  FUNC_ID int primary key AUTO_INCREMENT,
  FUNC_FUNCAO varchar(20) not null,
  FUNC_NOME varchar(200) not null,
  FUNC_DATANASC date not null,
  FUNC_TELEFONE int

);

CREATE TABLE IF NOT EXISTS VOOS (
  VOO_CODIGO INT primary key AUTO_INCREMENT,
  DATA_HORA_ORIGEM timestamp not null,
  DATA_HORA_DESTINO timestamp not null,
  AEROPORTO_ORIGEM int not null,
  AEROPORTO_DESTINO int not null,
  AERONAVE_PREFIXO int not null,
  CONSTRAINT FK_V_AEROPORTO FOREIGN KEY(AEROPORTO_ORIGEM) REFERENCES AEROPORTOS(AEROPORTO_COD),
  CONSTRAINT FK_V2_AEROPORTO FOREIGN KEY(AEROPORTO_DESTINO) REFERENCES AEROPORTOS(AEROPORTO_COD),
  constraint FK_AERONAVES foreign key(AERONAVE_PREFIXO) references AERONAVES(AERONAVE_PREFIXO)
  
);

create table if not exists EQUIPES (

  FUNC_ID int,
  VOO_CODIGO int,
  PRIMARY KEY(FUNC_ID, VOO_CODIGO),
  constraint FK_E_FUNCIONARIOS foreign key(FUNC_ID) references FUNCIONARIOS(FUNC_ID),
  constraint FK_E_VOO foreign key(VOO_CODIGO) references VOOS(VOO_CODIGO)  
);

create table if not exists RESERVAS (

  PASS_CPF int,
  VOO_CODIGO int
);

--
-- Alter Tables
--

alter table PASSAGEIROS ADD PASS_CEP VARCHAR(9);

alter table FUNCIONARIOS add FUNC_EMAIL VARCHAR(50);

alter table RESERVAS ADD constraint FK_R_VOOS foreign key(VOO_CODIGO) references VOOS(VOO_CODIGO);

alter table RESERVAS ADD constraint FK_R_PASSAGEIROS foreign key(PASS_CPF) references PASSAGEIROS(PASS_CPF);


-- Alter Table de cada tabela
ALTER TABLE PASSAGEIROS 
  ADD PASS_CNPJ VARCHAR(9);

ALTER TABLE PASSAGEIROS 
  DROP COLUMN PASS_CNPJ;

ALTER TABLE PASSAGEIROS 
  MODIFY COLUMN PASS_TELEFONE BIGINT;

ALTER TABLE PASSAGEIROS 
  CHANGE COLUMN PASS_RUA PASS_LOGRADOURO VARCHAR(45);


ALTER TABLE AEROPORTOS 
  ADD COLUMN AEROPORTO_TIPO VARCHAR(30);

ALTER TABLE AEROPORTOS 
  DROP COLUMN AEROPORTO_TIPO;

ALTER TABLE AEROPORTOS 
  MODIFY COLUMN AEROPORTO_LOCAL VARCHAR(150);

ALTER TABLE AEROPORTOS 
  CHANGE COLUMN AEROPORTO_LOCAL AEROPORTO_CIDADE VARCHAR(150);


ALTER TABLE AERONAVES 
  ADD COLUMN AERONAVE_PESO DECIMAL(10, 2);

ALTER TABLE AERONAVES 
  DROP COLUMN AERONAVE_PESO;

ALTER TABLE AERONAVES 
  MODIFY COLUMN AERONAVE_CAPACIDADE SMALLINT;

ALTER TABLE AERONAVES 
  CHANGE COLUMN AERONAVE_MODELO AERONAVE_TIPO VARCHAR(100);


ALTER TABLE FUNCIONARIOS 
  ADD FUNC_CNPJ VARCHAR(50);

ALTER TABLE FUNCIONARIOS 
  DROP COLUMN FUNC_CNPJ;

ALTER TABLE FUNCIONARIOS 
  MODIFY COLUMN FUNC_TELEFONE BIGINT;

ALTER TABLE FUNCIONARIOS 
  CHANGE COLUMN FUNC_FUNCAO FUNC_CARGO VARCHAR(20);


ALTER TABLE VOOS 
  ADD VOO_STATUS ENUM('programado', 'cancelado', 'finalizado');

ALTER TABLE VOOS 
  DROP COLUMN VOO_STATUS; -- Exemplo de DROP

ALTER TABLE VOOS 
  MODIFY COLUMN DATA_HORA_ORIGEM DATETIME NOT NULL;

ALTER TABLE VOOS 
  CHANGE COLUMN DATA_HORA_DESTINO DATA_HORA_CHEGADA TIMESTAMP;


ALTER TABLE EQUIPES 
  ADD COLUMN EQUIPE_FUNCAO VARCHAR(50);

ALTER TABLE EQUIPES 
  DROP COLUMN EQUIPE_FUNCAO;

ALTER TABLE EQUIPES 
  MODIFY COLUMN FUNC_ID INT SIGNED;

ALTER TABLE EQUIPES 
  CHANGE COLUMN VOO_CODIGO CODIGO_VOO INT;


ALTER TABLE RESERVAS 
  ADD RESERVA_DATA DATE;

ALTER TABLE RESERVAS 
  DROP COLUMN RESERVA_DATA; 

ALTER TABLE RESERVAS 
  MODIFY COLUMN PASS_CPF int; 

ALTER TABLE RESERVAS 
  CHANGE COLUMN VOO_CODIGO CODIGO_DO_VOO INT; 
